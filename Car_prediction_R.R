...
x = c("ggplot2", "corrgram", "DMwR", "usdm", "caret", "randomForest", "e1071",
      "DataCombine", "inTrees", "rpart.plot", "rpart",'MASS')
#load Packages
...
#Fare amount has a negative value.A price amount cannot be -ve and also cannot be 0. So we will remove these fields.
...
# pickup_weekdate has p value greater than 0.05 
...
#MAE : 3.1165,  MSE : 15.7470,  rmse:3.9682, MAPE: 0.4154
#Error rate : 41.54%
#accuracy: 58.46%
...
#MAE: 108981 MSE:6.703 RMSE: 2.589 MAPE: 0.2241
#Error rate: 22.41
#accuracy : 77.59%

###################Random Forest########################
rf_model = randomForest(fare_amount ~.,data=train_data, importance=TRUE,ntree=10)
...
#MAE:2.0744  MSE:7.4687  RMSE:2.7329   MAPE: 0.2548
#Error rate: 25.48, Accuracy: 74.52%

#increasing the number of trees count
rf_model50 = randomForest(fare_amount ~.,data=train_data, importance=TRUE,ntree=50)

summary(rf_model50)

rf_predictions = predict(rf_model50,test_data[,2:6])
...
rf_model100 = randomForest(fare_amount ~.,data=train_data, importance=TRUE,ntree=100)

summary(rf_model100)

rf_predictions = predict(rf_model100,test_data[,2:6])
...
#mae      mse       rmse         mape
#1.6183   5.1096    2.260       0.1861
######################Final submission############################
#we will train our model on whole training Dataset and save that model for further use
...
#Loading the problem test data
df_test=read.csv("test.csv",header = T,sep = ",")

xgb_pred = data.frame(df_test,"predictions" = xgb)
# writing the predicted fare_amount in disk as .csv file 
write.csv(xgb_pred,"carPrediction_R.csv",row.names = FALSE)
